<template>
  <div class="js-upload">
    ...
    <div v-if="fileName"
      ...
      <a class="filename uk-flex uk-flex-middle"
         :href="fileUrl"
         target="_blank">
        <div class="thumbnail uk-margin-small-right">
          <span v-if="!this.hasThumbnailUrl()"
                v-html="feather.icons['file'].toSvg({ width: 24, height: 24 })"></span>
          <img v-if="this.hasThumbnailUrl()"
               :src="this.actualThumbnailUrl" />
        </div>
      ...
      <button class="close-button"
              v-html="feather.icons['x'].toSvg({ width: 20, height: 20 })"
              v-on:click.prevent="clearFile"></button>
    ...
    <div class="select-files uk-margin">
      <button class="select-files-button"
              v-on:click.prevent="selectFilefromStorage">
        <span v-html="feather.icons['upload-cloud'].toSvg({ width: 18, height: 18 })"></span> Datei auswählen
      </button>
      <div v-if="filesFromStorage"
           id="files-selection-storage"
           class="uk-modal-container"
           uk-modal>
        <div class="uk-modal-dialog uk-modal-body">
          <h2 class="uk-modal-title">Datei Auswahl</h2>
          <div class="uk-child-width-1-2 uk-child-width-1-3@s uk-grid-small uk-grid-match"
               uk-grid>
            <div v-for="fileFromStorage of filesFromStorage"
              ...
              <div class="flex-end">
                ...
                <div class="uk-card-footer">
                  <button class="uk-button uk-button-primary uk-width-1-1 uk-margin-small-bottom"
                          v-on:click.prevent="selectFile(fileFromStorage)">Auswählen
                  </button>
                </div>
              </div>
            </div>
    ...
    <div v-if="loading"
         ...
         style="position: absolute; top: 0; right: 0; bottom: 0; left: 0; background: rgba(255,255,255,0.75);">
